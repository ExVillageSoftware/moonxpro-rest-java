...
public class WebSocketSession extends WebSocketListener {
    ...
    public synchronized void unSubscribe(Subscription s, StreamListener l) {
        Optional.ofNullable(listeners.get(s.streamKey)).ifPresent(o -> o.remove(l));
        List<Subscription> subs = subscriptions.computeIfAbsent(l, o -> new CopyOnWriteArrayList<>());
        subs.remove(s);
        if (subs.size() == 0) {
    ...
    public void onOpen(WebSocket webSocket, Response response) {
        ...
        String json = new JSONObject()
                .fluentPut(Field.Type.value(), "api-auth")
    ...
    public void onMessage(WebSocket webSocket, String text) {
        ...
        log("message :: " + o);
        Type type = Type.parse(o.getString(Field.Type.value()));
        switch (type) {
            case Snapshot:
            case DeltaUpdate:
                StreamKey streamKey = StreamKey.parse(o.getString(Field.StreamName.value()));
                Data data = new Data(type, streamKey, o.getLong(Field.SeqNo.value()), o.get(Field.Data.value()));
                Optional.ofNullable(listeners.get(streamKey))
                        .ifPresent(ls -> ls.forEach(l -> l.data(data)));
                break;
            case ActivateAlert:
                socket.send(new JSONObject()
                        .fluentPut(Field.Type.value(), Type.Activate.value())
            ...
            case Response:
                ...
                if (requestTag.equals(authRequestTag)) {
